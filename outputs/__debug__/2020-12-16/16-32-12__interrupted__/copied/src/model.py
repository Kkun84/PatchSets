...
class Model(pl.LightningModule):
    def __init__(
        ...
        self.encoder_linear_1 = nn.Linear(patch_size ** 2, hidden_n)
        self.encoder_linear_2 = nn.Linear(hidden_n, feature_n)

        self.decoder_linear_1 = nn.Linear(feature_n, hidden_n)
        self.decoder_linear_2 = nn.Linear(hidden_n, output_n)
    ...
    def encode(
        ...
        if isinstance(patch_set, Tensor):
            ...
            x = self.flatten(x)
            x = self.encoder_linear_1(x).relu()
            x = self.encoder_linear_2(x)
            x = x.reshape(*patch_set.shape[:2], *x.shape[2:])
        elif isinstance(patch_set, list):
            x = patch_set
            x = [self.flatten(i) for i in x]
            x = [self.encoder_linear_1(i).relu() for i in x]
            x = [self.encoder_linear_2(i) for i in x]
        else:
            assert False
        return x
    ...
    def decode(self, features: Tensor) -> Tensor:
        x = features
        x = self.decoder_linear_1(x).relu()
