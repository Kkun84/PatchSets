...
def test_train_make_patch1d(channels, batch_size, patch_size, patch_n):
    ...
    y = make_patch1d(x, patch_size, patch_n)
    logger.debug(f"x.shape={x.shape}")
    logger.debug(f"y.shape={y.shape}")
    assert list(y.shape) == [batch_size, patch_n, channels, *[patch_size] * 1]
...
def test_train_make_patch2d(channels, batch_size, patch_size, patch_n):
    ...
    x = torch.arange(np.prod(shape)).reshape(shape)
    y = make_patch2d(x, patch_size, patch_n)
    logger.debug(f"x.shape={x.shape}")
    logger.debug(f"y.shape={y.shape}")
    assert list(y.shape) == [batch_size, patch_n, channels, *[patch_size] * 2]
...
def test_train_make_patch3d(channels, batch_size, patch_size, patch_n):
    ...
    x = torch.arange(np.prod(shape)).reshape(shape)
    y = make_patch3d(x, patch_size, patch_n)
    logger.debug(f"x.shape={x.shape}")
    logger.debug(f"y.shape={y.shape}")
    assert list(y.shape) == [batch_size, patch_n, channels, *[patch_size] * 3]
